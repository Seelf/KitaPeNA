...
function restoreStateFromTab(tab) {
    if (tab.type === 'MIS') {
        ...
        if (tab.data.camera) {
            // We need to update the PETRI camera in state.js
            if (state.petriCamera) {
                state.petriCamera.x = tab.data.camera.x;
                state.petriCamera.y = tab.data.camera.y;
                state.petriCamera.zoom = tab.data.camera.zoom;
            }
        }

        // START REACHABILITY RESTORE
        nodes.length = 0;
        edges.length = 0;
        if (tab.data.nodes) tab.data.nodes.forEach(n => nodes.push(n));
        if (tab.data.edges) tab.data.edges.forEach(e => edges.push(e));
